...
describe('viewing a specific note', () => {
  test('succeeds with a valid id', async () => {
    ...
    const noteToView = await notesAtStart[0]
  ...
  test('fails with status code 404 if note does exist', async () => {
  ...
  test('fails with statuscode 400 id is invalid', async () => {
    const invalidId = '8sfhs43hg95g40hkvx8'
...
describe('addition of a new note', () => {
  test('succeeds with valid data', async () => {
    ...
    expect(noteAtEnd).toHaveLength(helper.initialNote.length + 1)
  ...
  test('fails with status code 400 if data invalid', async () => {
    ...
    const noteAtEnd = await helper.notesInDb()
    expect(noteAtEnd).toHaveLength(helper.initialNote.length)
  })
})
